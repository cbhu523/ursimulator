...
def optimize_word_layer(img_path, binarized_mask, original_score, word_list, word_scores, depth=1):
    if depth > MAX_DEPTH:
        return None, -np.inf

    space = [Categorical(word_list, name='word')]
    n_calls_val = BASE_N_CALLS + len(word_list) // 2
    result = gp_minimize(lambda word: objective(word, img_path, binarized_mask, original_score, word_scores), space, n_calls=n_calls_val, random_state=0)

    best_word = result.x[0]
    best_score = -result.fun
    
    similar_words = [word for word, _ in word2vec_model.most_similar(best_word, topn=NUM_SIMILAR_WORDS)]
    
    # Ensure the recursive call includes the word_scores argument
    next_best_word, next_best_score = optimize_word_layer(img_path, binarized_mask, original_score, similar_words, word_scores, depth=depth+1)
    
    return (next_best_word, next_best_score) if next_best_score > best_score else (best_word, best_score)
